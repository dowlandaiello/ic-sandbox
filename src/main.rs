...
fn main() {
    ...
    let cmd = clap::Command::new("icc")
        ...
        .subcommand(
            Command::new("compile")
                .about(
                    "Parses an input .inet file, producing a bincode representation in the specified out file",
                )
                .arg(arg_in_file())
        ...
        .subcommand(
            Command::new("eval")
                .about(
                    "Parses an input .inet file, reducing the input to completion and echoing the reduced expression, if not out file is specified",
                )
                .arg(arg_in_file())
                .arg(arg_out_file_default("STDOUT".into())),
        )
        .subcommand(
            Command::new("dev").about("Interactively evaluates interaction nets"),
        );
